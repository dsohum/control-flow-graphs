...
%union 
{
	...
	list<string>* names;
	list<pair<Ast*,Sequence_Ast*> >* cases;
};
...
%token IF
%token FOR
%token SWITCH DEFAULT CASE BREAK
...
%type <ast> statement
%type <sequence_ast> assignment_list
...
|
	...
	if (NOT_ONLY_PARSE)
	{
		CHECK_INVARIANT(($1 != NULL), "switch-statement cannot be null");	
		$$ = $1;
	}		
	}
|
...
switch_statement:
	...
	if (NOT_ONLY_PARSE)
	{
		CHECK_INVARIANT(($3 != NULL), "arithmetic expression cannot be null");			
		CHECK_INVARIANT(($6 != NULL), "case list cannot be null");			
		list<pair<Ast*,Sequence_Ast*> >* case_list = $6;
		if(case_list->size()==0){
			$$ = new Sequence_Ast(get_line_number());
		} else if(case_list->size()==1){
			if (case_list->front().first==NULL) {
				$$ = case_list->front().second;
			} else {
				Relational_Expr_Ast* rel_exp = new Relational_Expr_Ast($3,equalto,case_list->front().first,get_line_number());
				Sequence_Ast* empty_else_ast = new Sequence_Ast(get_line_number());
				Selection_Statement_Ast* if_then = new Selection_Statement_Ast(rel_exp,case_list->front().second,empty_else_ast,get_line_number());
				$$ = if_then;
			}
		} else {
			Ast* else_ast;
			if (case_list->front().first==NULL) {
				else_ast = case_list->front().second;
			} else {
				Relational_Expr_Ast* rel_exp = new Relational_Expr_Ast($3,equalto,case_list->front().first,get_line_number());
				Sequence_Ast* empty_else_ast = new Sequence_Ast(get_line_number());
				else_ast = new Selection_Statement_Ast(rel_exp,case_list->front().second,empty_else_ast,get_line_number());
			}
			list<pair<Ast*,Sequence_Ast*> >::iterator it = case_list->begin();
			for(it++;it!=case_list->end();it++){
				Relational_Expr_Ast* rel_exp = new Relational_Expr_Ast($3,equalto,it->first,get_line_number());
				else_ast = new Selection_Statement_Ast(rel_exp,it->second,else_ast,get_line_number());				
			}
			$$=else_ast;
		}
	}
...
case_list:
	...
	if (NOT_ONLY_PARSE)
	{
		list<pair<Ast*,Sequence_Ast*> >* case_list = new list<pair<Ast*,Sequence_Ast*> >();
		$$ =	case_list;
	}
	}	
|
	...
	if (NOT_ONLY_PARSE)
	{
		CHECK_INVARIANT(($3 != NULL), "statement list cannot be null");			
		list<pair<Ast*,Sequence_Ast*> >* case_list = new list<pair<Ast*,Sequence_Ast*> >();
		case_list->push_back(pair<Ast*,Sequence_Ast*>(NULL,$3));
		$$ =	case_list;
	}
	}
|
	CASE arith_expression ':' statement_list BREAK ';' case_list 
	{
	if (NOT_ONLY_PARSE)
	{
		CHECK_INVARIANT(($2 != NULL), "arithmetic expression cannot be null");			
		CHECK_INVARIANT(($4 != NULL), "statement list cannot be null");			
		CHECK_INVARIANT(($7 != NULL), "case list cannot be null");			
		list<pair<Ast*,Sequence_Ast*> >* case_list = $7;
		case_list->push_back(pair<Ast*,Sequence_Ast*>($2,$4));
		$$ =	case_list;
	}
	}
;
...
|	FOR '(' assignment_list ';' bool_expression_term ';' assignment_list ')' statement
	...
	if (NOT_ONLY_PARSE)
	{
		CHECK_INVARIANT(($3 != NULL), "assignment-list cannot be null");	
		CHECK_INVARIANT(($5 != NULL), "condition-expression cannot be null");	
		CHECK_INVARIANT(($7 != NULL), "assignment-list cannot be null");	
		CHECK_INVARIANT(($9 != NULL), "body-expression cannot be null");	
	
		Sequence_Ast* for_ast = $3;
		Sequence_Ast* body_ast = new Sequence_Ast(get_line_number());
		body_ast->ast_push_back($9);
		body_ast->ast_push_back($7);
		Iteration_Statement_Ast* while_stmt = new Iteration_Statement_Ast($5,body_ast,get_line_number(),false);
		for_ast->ast_push_back(while_stmt);
		$$ = (Ast*)for_ast;
	}
	}
;
...
|
	...
	if (NOT_ONLY_PARSE)
	{
		...
		Sequence_Ast *assign_list = new Sequence_Ast(get_line_number());
		assign_list->ast_push_back((Ast*)assign_ast);
		$$=assign_list;			//assign_ast->check_ast();
	}
	}
|
	...
	if (NOT_ONLY_PARSE)
	{
		...
		CHECK_INVARIANT(( assign_list!= NULL), "assignment list cannot be null");
		CHECK_INVARIANT(((var_ast != NULL) && (arith_exp_ast != NULL)), "lhs/rhs cannot be null");
		Assignment_Ast *assign_ast = new Assignment_Ast(var_ast,arith_exp_ast,get_line_number());
		assign_list->ast_push_back(assign_ast);
